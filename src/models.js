...
const meetings = {
  ...
  nextUnified: (from = new Date()) => {
    ...
    const oneOff = meetings.next(from);
    if (oneOff) {
      candidates.push({
        ...oneOff,
        is_recurring: false,
...
const recurring = {
  ...
  nextOccurrence: (rule, from = new Date()) => {
    ...
    const skipped = new Set(recurring.skipDates(rule.id, skipFloor));
...
const hosts = {
   ...
   * hosting assignment between "now" (minus a linger allowance) and 30 days
   * out? This is what loadUser calls per request to publish res.locals.isHost
   * — which gates /timer access, not just a view hint (see src/routes/timer.js)
   * — so it has to stay this cheap AND must not go false while a member is
   * mid-session, or a host running the timer would lose access to their own
   * clock.
   *
  ...
  hasUpcoming: (userId, days = 30) => {
    ...
    const row = db
      .prepare(
        `SELECT
           EXISTS (SELECT 1 FROM occurrence_hosts
                    WHERE user_id = @uid AND local_date >= @recurFloor AND local_date <= @until)
           OR
      ...
      .get({ uid: userId, recurFloor, until, floorIso, untilIso });
    return !!(row && row.yes);
  },
